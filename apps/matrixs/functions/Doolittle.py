...
def formaMatrizAumentada(A,b):
        ...
        if ranA == ranAb and ranAb == n:
            ...
            result += f"The range of A is equal to the range of the augmented matrix but the range of the augmented matrix is ​​less than the number of unknowns, then the system is indeterminate compatible and for this reason the system has infinite solutions, also the determinant of the matrix is ​​{det :.5f} and therefore the method does not converge</br>"
        else:
...
def doolittle(A, b):
        ...
        html = f"</br>Doolittle</br></br>Results</br></br>Stage {etapa}</br></br>"
        for i in A:
            result = ""
            for j in i:
                result += f"{j:.10e} "
